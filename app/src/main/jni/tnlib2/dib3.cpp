...
WORD FAR DIBNumColors( BITMAPINFOHEADER *lpDIB )
{
   ...
   if (IS_WIN30_DIB((LPBITMAPINFOHEADER)lpDIB))
   {
      DWORD dwClrUsed;
...
HPALETTE FAR CreateDIBPalette(HDIB hDIB)
{
   ...
   bWinStyleDIB = IS_WIN30_DIB((LPBITMAPINFOHEADER)lpbi);
   if (wNumColors)
   {
...
HBITMAP FAR DIBToBitmap(HDIB hDIB, HPALETTE hPal)
{
   ...
   lpDIBBits = FindDIBBits((BITMAPINFOHEADER*)lpDIBHdr);
...
HDIB FAR BitmapToDIB(HBITMAP hBitmap, HPALETTE hPal)
{
   ...
   *lpbi = bi;
   ...
   if ( ( h = (HDIB)GlobalReAlloc(hDIB, dwLen, 0) ) != NULL )
      hDIB = h;
   else
   {
      /* clean up and return NULL */
...
//      hDIB = NULL;
   ...
   if (GetDIBits(hDC, hBitmap, 0, (WORD)bi.biHeight, (LPSTR)lpbi + (WORD)lpbi
         ->biSize + PaletteSize(lpbi), (LPBITMAPINFO)lpbi,
         DIB_RGB_COLORS) == 0)
   {
      /* clean up and return NULL */
      GlobalUnlock(hDIB);
...
HANDLE AllocRoomForDIB(BITMAPINFOHEADER bi, HBITMAP hBitmap)
{
   ...
   *lpbi = bi;
   ...
   dwLen = lpbi->biSize + PaletteSize(&bi) + lpbi->biSizeImage;

   /* Unlock the memory block */
   GlobalUnlock(hDIB);

   /* ReAlloc the buffer big enough to hold all the bits */
   if ( ( hTemp = (HANDLE)GlobalReAlloc(hDIB,dwLen,0) ) != NULL )
      return hTemp;
   else
      {
...
//	DWORD DIBComp = ((LPBITMAPINFOHEADER)lpbi)->biCompression;	// DIB compression
   GlobalUnlock(hDIB);
