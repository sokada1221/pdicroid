...
int HitTestX(TNFONT &tnfont, TDispLineInfo &linfo, const tchar *str, const RECT &rc, UINT flags, CharHT &cht, const TDrawSetting &setting)
{
	...
	int cx = linfo.pos.x;	// １行のｘドット数
	const tchar *p = str + linfo.loc;	// 現在の処理文字列
//	const tchar *leftp = p;
	const tchar *endp = p + linfo.length;

	const int tabwidth = linfo.pfont->GetSpcWidth() * setting.TabCol;
#ifndef _UNICODE
	bool fSingleByte = linfo.pfont->fSingleByte;
#endif
...
j0:;
		...
		jmp111:;
#endif
			//DBW("cx=%d %d %d",cx,w,STR_DIFF(p,str+lineinfo.loc));
		} else {
			// 最終端の場合の処理
			c = 0;
			nextp = p;
		...
		// Hit Test
		POINT pt = cht.pt;
		if (PointRange(pt.x, rc.left+cx, rc.left+cx + w)){
			if ( flags & DF_WHOLECHAR ){
				return STR_DIFF( p, str );
			} else {
				if ( pt.x >= rc.left + cx + (w>>1) ){
...
// parser for one line info.
// str : top of the text.
int HitTestX(TNFONT &tnfont, TDispLineInfo &linfo, const tchar *str, const RECT &rc, UINT flags, EnphTextWork &etw, TDrawSetting &setting)
{
	HDC hdc = tnfont;
