...
HANDLE ReadDIBFile( int , BITMAPINFOHEADER *lpbih = NULL );
BOOL MyRead( HFILE, LPSTR, DWORD);
BOOL SaveDIBFile(void);
BOOL WriteDIB(LPSTR, HANDLE);
DWORD PASCAL MyWrite( HFILE, VOID FAR *, DWORD);
...
HDIB FAR LoadDIB( const char *lpFileName )
{
   HDIB hDIB;
   int hFile;
   OFSTRUCT ofs;
...
//	SetCursor(LoadCursor(NULL, IDC_WAIT));
   if ((hFile = OpenFile(lpFileName, &ofs, OF_READ)) != HFILE_ERROR )
   {
...
//	SetCursor(LoadCursor(NULL, IDC_WAIT));
   if ((hFile = OpenFile(lpFileName, &ofs, OF_READ)) != HFILE_ERROR )
   {
	  BOOL r = (BOOL)ReadDIBFile( hFile, lpbih );
	  _lclose(hFile);
	  return r;
   }
   else
...
WORD FAR SaveDIB(HDIB hDib, LPSTR lpFileName)
{
   ...
   dwDIBSize = lpBI->biSize + PaletteSize(lpBI);  // Partial Calculation
   ...
   if (dwError == 0)
     return DIBERR_OPEN; // oops, something happened in the write
   else
...
// lpbihにBITMAPINFOHEADER構造体を渡すとビットマップの情報だけを返す 
HANDLE ReadDIBFile( int hFile, BITMAPINFOHEADER *lpbih )
{
...
//	DWORD dwBitsSize = filelength(hFile);
   ...
   if (lpbi->biSize == sizeof(BITMAPCOREHEADER))
	 ...
	 // 情報を得るだけの場合
	 if ( lpbih ){
	 	memcpy( lpbih, lpbi, sizeof(BITMAPINFOHEADER) );
		GlobalUnlock(hDIB);
		GlobalFree(hDIB);
		return (HANDLE)TRUE;
	 }
   ...
   if ((nNumColors = (UINT)lpbi->biClrUsed)==NULL)
    {
   ...
   _lread(hFile, (LPSTR)(lpbi) + (int)lpbi->biSize, nNumColors * sizeof(RGBQUAD));
   ...
   if (bmfHeader.bfOffBits != 0L)
      _llseek(hFile, bmfHeader.bfOffBits, 0 );	// SEEK_SET
   
   if (MyRead(hFile, (LPSTR)lpbi + (int)offBits, lpbi->biSizeImage))
     goto OKExit;
...
#ifdef WIN32
BOOL MyRead( HFILE hFile, LPSTR lpBuffer, DWORD dwSize)
{
	if (_lread( hFile, lpBuffer, dwSize ) != dwSize )
		 return FALSE;
   return TRUE;
}
#else
...
#ifdef WIN32
DWORD PASCAL MyWrite( HFILE iFileHandle, VOID FAR *lpBuffer, DWORD dwBytes)
{
	if (_lwrite( iFileHandle, (LPSTR)lpBuffer, dwBytes ) != dwBytes )
		return 0;
   return dwBytes;
}
#else
