...
static const tchar *FindDelimiter(const tchar *leftp, int &delimtype, vector<int> &wordindex, TFontAttr *fa, const tchar **tagtop, TFontTagStack &faStack, THyperLinks *hls __arg_asc(bool fSingleByte));
static const tchar *FindBreakPoint(vector<int> &wordindex, int wordindex_offs, const tchar *leftp, int end);

const tchar *ParseHtml(const tchar *tagp, TFontAttr &fa, TFontTagStack &faStack, THyperLinks *hls);
const tchar *ParseHtmlTag(const tchar *text, const tchar **name_end, tnstr_map *attrs);

const tchar *ParsePdicHyperLink(const tchar *tagp);
...
static void AssignFont(HDC hdc, TDispLineInfo &info, TFontAttrMap &FontAttrs, TFontAttr &faNext)
{
...
void ParseText(TNFONT &org_tnfont, TDispLinesInfo &linesinfo, const tchar *str, RECT *rc, TDrawSetting &setting, TFontAttrMap &FontAttrs, THyperLinks *hls)
{
	...
	TFontAttr faNext(info.FontAttr);
	...
	while (*leftp){
		...
		if (setting.HtmlParse){
			if (delimtype & (DMT_HYPLINK1|DMT_HYPLINK2)){
				...
				nextp = FindDelimiter(leftp, delimtype, wordindex, &faNext, &endp, linesinfo.FAStack, hls __arg_asc(fSingleByte));
				if (!endp)
					endp = nextp;
			}
		} else {
			endp = nextp = FindDelimiter(leftp, delimtype, wordindex, NULL, NULL, linesinfo.FAStack, hls __arg_asc(fSingleByte));
		}
...
jnext:;
			...
			int length = STR_DIFF(next_leftp,leftp);
			int tlength = STR_DIFF(text_endp,leftp);
			if (leftp==endp){
				// １文字も表示できない or 改行のみ
				if (!(delimtype_cur&DMT_CR)){
					...
					if (textlen==0){
						// delim charと仮定
						__assert((delimtype&DMT_HYPLINK2) || (delimtype!=0 && leftp!=nextp));
						leftp = nextp;
					} else {
						leftp += textlen;
					}
...
const tchar *FindDelimiter(const tchar *leftp, int &delimtype, vector<int> &wordindex, TFontAttr *fa, const tchar **tagtop, TFontTagStack &faStack, THyperLinks *hls __arg_asc(bool fSingleByte))
{
	delimtype = DMT_TERMINAL;
	ushort c;
						...
						if (fa){
							...
							if (c=='<'){
								*tagtop = p;
								do {
									// HTML?
									nextp2 = ParseHtml(p, *fa, faStack, hls);
									if (nextp2==p){
										// Not HTML or end
										// if (PdicHyperLink enabled)
										nextp2 = ParsePdicHyperLink(p);
										if (!nextp2){
											// No hyperlink
											break;
										}
...
const tchar *ParseHtml(const tchar *tagp, TFontAttr &fa, TFontTagStack &faStack, THyperLinks *hls)
{
	...
	if (!_tcsnicmp(nextp, _t("font"), name_len)){
		if (on){
			...
			if (attrs.count(_t("color"))){
				COLORREF color = ParseColor(attrs[_t("color")]);
				if (color!=(COLORREF)-1){
					fa.Color = color;
				}
			}
			...
			if (attrs.count(_t("bgcolor"))){
				COLORREF color = ParseColor(attrs[_t("bgcolor")]);
				if (color!=(COLORREF)-1){
					fa.BgColor = color;
				}
			}
...
static void setTagAttr(tnstr_map &attrs, const tchar *name_top, const tchar *name_tail, const tchar *val_top, const tchar *val_tail)
{
	...
	tnstr val(val_top, STR_DIFF(val_tail, val_top));
	attrs[key] = val;
}

static void setTagAttr(tnstr_map &attrs, const tchar *name_top, const tchar *name_tail)
{
	tnstr key(name_top, STR_DIFF(name_tail, name_top));
	tolower(key);
	attrs[key] = _t("1");
...
// Parse "<...>"
const tchar *ParseHtmlTag(const tchar *text, const tchar **name_end, tnstr_map *attrs __arg_asc(bool fSingleByte))
{
			...
			if (quoted){
				...
				switch (c){
					...
					case '>':
						if (attrs){
							if (val_top){
								setTagAttr(*attrs, name_top, name_tail, val_top, p);
							}
#if 0
...
{
	{ _T("word:"), 5, 5 },
	{ _T("→"), BYTETOLEN(2), BYTETOLEN(2) },
	{ _T("http:"), 5, 0 },
	{ _T("https:"), 6, 0 },
	{ _T("mailto:"), 7, 0 },
	{ _T("file:"), 5, 5 },
	{ _T("text:"), 5, 5 },
//	{ _T("html:"), 5 },	// HTMLのpop-up
};

// text : points to the top of the tag.
// return : points to the top of the link address.
const tchar *ParsePdicHyperLink(const tchar *text)
{
	text++;	// Skip '<'.
