...
{
	tstring_vec *vec = new tstring_vec;
	tstring buf(str);
	TCHAR *p = _tcstok(const_cast<TCHAR*>(buf.c_str()), delim);
	while (p){
		vec->push_back(p);
...
void split(const TCHAR *str, tstring_vec &array, const TCHAR *delim)
{
	...
	tstring s(str);
	const TCHAR *p = _tcstok(const_cast<TCHAR*>(s.c_str()), delim);
	for (;p!=NULL;){
		array.push_back(p);
		p = _tcstok(NULL, delim);
	}
...
bool IsDirectory(const tchar *dir)
{
	WIN32_FIND_DATA wfd;
...
#if 0	//TBD:
time_t st_mtime(const tchar *file)
{
